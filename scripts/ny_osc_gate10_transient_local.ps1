...
$LocalApproval = Join-Path $RepoRoot "sources\evidence\ny_osc_owner_name_file_tenth_attempt_transient_local_approval.v1.json"
$PiiApproval = Join-Path $RepoRoot "sources\evidence\ny_osc_owner_name_file_tenth_attempt_transient_pii_approval.v1.json"
$FreshPreflightReceipt = Join-Path $RepoRoot "sources\evidence\ny_osc_owner_name_file_tenth_fresh_listing_preflight_receipt.v1.json"
$ExecutionAuthorization = Join-Path $RepoRoot "sources\evidence\ny_osc_owner_name_file_tenth_execution_authorization.v1.json"
$PythonEntrypoint = Join-Path $RepoRoot "scripts\ny_osc_gate10_execute.py"

$ExpectedProposalRef = "sources/proposals/ny_osc_owner_name_file_tenth_product_slice_authorization.v1.json"
$ExpectedProposalCheckpoint = "dea34cbc503dd5f6bf62ec6c31483056c4b34966"
$ExpectedAttemptNumber = 10
$ExpectedPreflightFreshnessSeconds = 900
$ExpectedLocalAuthorization = "APPROVO NY OSC TENTH TRANSIENT LOCAL FILE BOUNDED ONCE"
$ExpectedPiiAuthorization = "APPROVO NY OSC OWNER NAME FILE TENTH BOUNDED TRANSIENT PII ATTEMPT ONCE"
$ExpectedExecutionAuthorization = "AUTHORIZE_NY_OSC_TENTH_BOUNDED_EXECUTION_ONCE"

$ProtectedGate10Paths = @(
    ...
    "src/unclaimed_platform/adapters/sources/ny_owner_name_transient_local_execution_v1_7.py"
    "src/unclaimed_platform/domain/mvp1_vertical_slice.py"
)

function Assert-Gate10ProtectedPackage {
    param([Parameter(Mandatory = $true)][string]$RunnerCheckpoint)
    if ($RunnerCheckpoint -notmatch '^[0-9a-f]{40}$') { throw "Attempt-10 runner checkpoint is invalid. Do not download." }
    $Inside = (& git -C $RepoRoot rev-parse --is-inside-work-tree 2>$null)
    if ($LASTEXITCODE -ne 0 -or $Inside.Trim() -ne "true") { throw "Gate 10 must run from the authorized Git worktree. Do not download." }
    & git -C $RepoRoot cat-file -e "${RunnerCheckpoint}^{commit}" 2>$null
    if ($LASTEXITCODE -ne 0) { throw "Approved Gate 10 checkpoint is unavailable locally. Do not download." }
    foreach ($RelativePath in $ProtectedGate10Paths) {
        $ExpectedBlob = (& git -C $RepoRoot rev-parse "${RunnerCheckpoint}:$RelativePath" 2>$null)
        $ActualPath = Join-Path $RepoRoot $RelativePath
        if ($LASTEXITCODE -ne 0 -or -not (Test-Path -LiteralPath $ActualPath -PathType Leaf)) { throw "Protected Gate 10 path is unavailable. Do not download." }
        $ActualBlob = (& git -C $RepoRoot hash-object -- $ActualPath 2>$null)
        if ($LASTEXITCODE -ne 0 -or $ActualBlob.Trim() -ne $ExpectedBlob.Trim()) { throw "Protected Gate 10 package differs from approved checkpoint. Do not download." }
    }
}
...
foreach ($RequiredArtifact in @($LocalApproval, $PiiApproval, $FreshPreflightReceipt, $ExecutionAuthorization)) {
    if (-not (Test-Path -LiteralPath $RequiredArtifact -PathType Leaf)) { throw "Required attempt-10 authorization artifact is missing. Do not download." }
}
...
if ($Local.status -ne "GRANTED_NOT_CONSUMED" -or $Pii.status -ne "GRANTED_NOT_CONSUMED" -or $Execution.status -ne "GRANTED_NOT_CONSUMED") {
    throw "All attempt-10 single-use approvals must be granted and unconsumed. Do not download."
}
...
Assert-Gate10ProtectedPackage -RunnerCheckpoint $RunnerCheckpoint
...
if ($Local.owner_authorization -ne $ExpectedLocalAuthorization -or $Pii.owner_authorization -ne $ExpectedPiiAuthorization -or $Execution.owner_authorization -ne $ExpectedExecutionAuthorization) {
    throw "Attempt-10 human authorization phrase mismatch. Do not download."
}
if ($Preflight.status -ne "EXACT_MATCH" -or $Preflight.remote_preflight_performed -ne $true -or $Preflight.freshness_window_seconds -ne $ExpectedPreflightFreshnessSeconds) {
    throw "Attempt-10 fresh preflight is invalid. Do not download."
}
if (
...
) { throw "Attempt-10 listing metadata is not an exact match. Do not download." }
if ($Preflight.download_performed -ne $false -or $Preflight.owner_file_opened -ne $false -or $Preflight.owner_pii_processed -ne $false -or $Preflight.contains_owner_pii -ne $false) {
    throw "Attempt-10 preflight privacy boundary mismatch. Do not download."
}
try { $PerformedAt = [DateTimeOffset]::Parse($Preflight.performed_at_utc) } catch { throw "Attempt-10 preflight timestamp is invalid. Do not download." }
$AgeSeconds = ([DateTimeOffset]::UtcNow - $PerformedAt.ToUniversalTime()).TotalSeconds
if ($AgeSeconds -lt 0 -or $AgeSeconds -gt $ExpectedPreflightFreshnessSeconds) { throw "Attempt-10 fresh preflight is stale or future-dated. Do not download." }

if ($Execution.fresh_preflight_receipt_ref -ne $Preflight.receipt_ref -or $Execution.fresh_preflight_status -ne "EXACT_MATCH" -or $Execution.download_authority -ne "ONE_MANUAL_DOWNLOAD_TO_DEDICATED_OS_TEMP" -or $Execution.execution_authority -ne "ONE_BOUND_GATE10_EXECUTION") {
    throw "Attempt-10 explicit execution authorization mismatch. Do not download."
}
if ($Local.scope.expected_local_filename -ne "FINDERS.zip" -or $Local.scope.max_download_bytes -ne 450000000 -or $Local.scope.immediate_logical_deletion_required -ne $true -or $Local.scope.durable_raw_persistence_allowed -ne $false -or $Local.scope.repository_persistence_allowed -ne $false -or $Local.scope.cloud_sync_allowed -ne $false -or $Local.scope.chat_upload_allowed -ne $false) {
    throw "Attempt-10 local retention scope mismatch. Do not download."
}
if ($Pii.execution_bounds.downloads_max -ne 1 -or $Pii.execution_bounds.retries_max -ne 0 -or $Pii.execution_bounds.max_download_bytes -ne 450000000 -or $Pii.execution_bounds.max_uncompressed_bytes -ne 2000000000 -or $Pii.execution_bounds.max_archive_members -ne 1 -or $Pii.execution_bounds.required_execution_result_contract_version -ne "1.7.0") {
    throw "Attempt-10 execution bounds mismatch. Do not download."
}
if ($Pii.processing_scope.owner_rows_persistence_allowed -ne $false -or $Pii.processing_scope.owner_field_logging_allowed -ne $false -or $Pii.processing_scope.row_specific_human_inspection_allowed -ne $false -or $Pii.processing_scope.property_type_code_only_buffering_allowed -ne $true) {
    throw "Attempt-10 processing scope mismatch. Do not download."
}
if ($Execution.authorization_scope.downloads_max -ne 1 -or $Execution.authorization_scope.gate10_executions_max -ne 1 -or $Execution.authorization_scope.retries_max -ne 0 -or $Execution.authorization_scope.direct_network_client_allowed -ne $false) {
    throw "Attempt-10 execution scope mismatch. Do not download."
}

$TempDir = Join-Path $env:TEMP ("unclaimed-ny-osc-gate2-tenth-" + [guid]::NewGuid().ToString("N"))
...
try {
    Write-Host ""
    Write-Host "NY OSC tenth bounded product-slice attempt"
    ...
    Write-Host "Attempt-10 download-start marker captured."
    Read-Host "Press Enter only after that same single download finishes"
    if (-not (Test-Path -LiteralPath $Archive -PathType Leaf)) { throw "FINDERS.zip not found in the dedicated attempt-10 temp directory." }
    ...
    if ($LASTEXITCODE -ne 0) { throw "NY OSC tenth bounded product-slice attempt stopped fail-closed." }
}
finally {
