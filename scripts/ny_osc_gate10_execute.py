"""Dedicated Python entrypoint for NY OSC Gate 10."""
...
def main(argv: list[str]) -> int:
    if len(argv) != 8:
        raise SystemExit("Gate 10 entrypoint requires exactly seven arguments")
    archive = Path(argv[1])
    authorization = build_real_execution_authorization_v1_6(
        Path(argv[2]),
        Path(argv[3]),
        Path(argv[4]),
        Path(argv[5]),
    ...
    result = execute_transient_local_product_slice_v1_7(authorization, archive)
