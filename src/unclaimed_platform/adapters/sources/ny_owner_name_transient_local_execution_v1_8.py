"""NY OSC attempt-11 bounded product-slice runtime v1.8."""
...
class NyTransientLocalExecutionAuthorizationV1_7(BaseModel):
    model_config = ConfigDict(frozen=True)

    contract_version: Literal["1.7.0"] = "1.7.0"
    mode: Literal["AUTHORIZED_REAL_ONCE"] = "AUTHORIZED_REAL_ONCE"
    attempt_number: Literal[11] = 11
    eleventh_transient_local_approval_ref: str = Field(min_length=1)
    eleventh_transient_pii_approval_ref: str = Field(min_length=1)
    eleventh_fresh_preflight_receipt_ref: str = Field(min_length=1)
    eleventh_execution_authorization_ref: str = Field(min_length=1)
...
class NyTransientLocalExecutionResultV1_8(BaseModel):
    model_config = ConfigDict(frozen=True)

    contract_version: Literal["1.8.0"] = "1.8.0"
    ...
    download_start_detection_mode: Literal[
...
def build_real_execution_authorization_v1_7(
    local_path: Path,
    pii_path: Path,
    preflight_path: Path,
    execution_path: Path,
    *,
...
) -> NyTransientLocalExecutionAuthorizationV1_7:
    ...
    if (
        execution.get("download_authority")
        != "ONE_MANUAL_DOWNLOAD_TO_DEDICATED_OS_TEMP"
    ):
        raise ValueError("download authority mismatch")
    if execution.get("execution_authority") != "ONE_BOUND_GATE11_EXECUTION":
        raise ValueError("Gate 11 execution authority mismatch")
    return NyTransientLocalExecutionAuthorizationV1_7(
        eleventh_transient_local_approval_ref=_string(
            local,
            "execution_approval_ref",
            "local approval",
        ),
        eleventh_transient_pii_approval_ref=_string(
            pii,
            "execution_approval_ref",
            "PII approval",
        ),
        eleventh_fresh_preflight_receipt_ref=preflight_ref,
        eleventh_execution_authorization_ref=_string(
            execution,
            "execution_approval_ref",
...
def execute_transient_local_product_slice_v1_8(
    authorization: NyTransientLocalExecutionAuthorizationV1_7,
    archive_path: Path,
) -> NyTransientLocalExecutionResultV1_8:
    ...
    try:
        ...
        return NyTransientLocalExecutionResultV1_8(
            status="COMPLETED",
            ...
            execution_authorization_ref=(
                authorization.eleventh_execution_authorization_ref
            ),
            authorized_download_started_at_utc=(
        ...
        try:
            ...
            if parent.name.startswith("unclaimed-ny-osc-gate2-eleventh-"):
